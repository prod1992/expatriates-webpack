$.ui.autocomplete.prototype._renderItem = function (ul, item) {
    ...
    var t = item.label.replace(re, "<span class='matched'>" + this.term + "</span>");
    return $("<li>").attr("data-value", item.label).append(t).appendTo(ul);
};
var _options = {
    delay: 200,
    create: function (e) {
        ...
        if (!!valueData) {
            $("<input type='hidden' id='" + valueData + "' name='" + valueData + "' value='" + defaultValue + "'/>")
                .appendTo($el.parent());
        }
    },
    select: function (e, ui) {
        ...
        $itemsEl.find("li[data-value=\"" + item.label + "\"]").addClass("ui-state-focus");
    ...
    focus: function (e, ui) {
        ...
        $(e.currentTarget).find("li").removeClass("ui-state-focus");
        $(e.currentTarget).find("li[data-value=\"" + item.label + "\"]").addClass("ui-state-focus");
        $valueEl.val(item.value);
    },
    search: function (e) {
        ...
        $(_src).each(function (i, el) {
            ...
            if (el.label.substr(0, sValue.length).toLowerCase() == sValue.toLowerCase()) {
                // ... add element
                arr.push(el);
            }
        });
